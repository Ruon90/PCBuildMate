...
def convert_from_usd(amount, currency_code):
    """Convert an amount in USD to the given currency code using CurrencyRate.rate_to_usd.

    amount: numeric or string representing USD amount
    currency_code: ISO code to convert to (e.g., 'EUR').

    Returns a Decimal rounded to 2 decimal places; on error returns the original amount.
    """
    try:
        amt = Decimal(amount)

...
class AlternativesViewTests(TestCase):
    def setUp(self):
        ...
        self.mobo = Motherboard.objects.create(name='Test Mobo', socket='AM4', price=120)
        self.ram = RAM.objects.create(name='Test RAM', ddr_generation='DDR4', price=80)
        self.storage = Storage.objects.create(name='Test NVMe', interface='nvme', price=60)
        self.psu = PSU.objects.create(name='Test PSU', wattage=650, price=90)
        self.cooler = CPUCooler.objects.create(name='Test Cooler', power_throughput=150, price=50)
        self.case = Case.objects.create(name='Test Case', case_type='ATX', price=70)
    ...
    def test_select_alternative_replaces_preview(self):
        session = self.client.session
        session['preview_alternatives'] = [self.alt]
        session['preview_build'] = {'budget': 1000, 'currency': 'USD', 'mode': 'gaming', 'resolution': '1440p'}
        session.save()

        resp = self.client.post(reverse('select_alternative'), {'alt_index': 0}, follow=True)
        self.assertEqual(resp.status_code, 200)

...
def extract_brand_and_model(name: str):
    ...
    brand = (
        "AMD"
        if s_lower.startswith("amd")
        else ("Intel" if s_lower.startswith("intel") else "Other")
    )
...
def clean_cpu(
    ...
    if df.empty or set(df.columns) & EXPECTED_HEADERS == set():
        raise ValueError(
            f"cpu.csv does not contain expected headers. Found: {list(df.columns)}"
        )
    ...
    if enrich_threads:
        ...
        df["thread_count"] = (
            df["core_count"]
            .apply(parse_int)
            .apply(lambda c: c * 2 if c is not None else None)
        )
    ...
    cols = ["brand", "model", "slug"] + [
        c for c in df.columns if c not in ["brand", "model", "slug"]
    ]
    df = df[cols]
    ...
    print(
        f"CPU cleaned dataset saved to {output_path} | rows={len(df)} | sep='{sep}'"
    )
